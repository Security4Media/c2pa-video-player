...
} from 'https://cdn.jsdelivr.net/npm/c2pa@0.30.11/+esm';
...
export var C2PAMenu = function () {
  ...
  return {
    ...
    c2paItem: function (itemName, c2paStatus, compromisedRegions = []) {
      ...
      try {
        validation_report = c2paStatus.details.video.validation_report;
        activeManifest = validation_report.manifests[validation_report.active_manifest];
        console.log('[C2PA] This is the manifest', validation_report);
      } catch (error) {
        console.error('[C2PA] Manifest does not exist');
      }
      if (validation_report != null && validation_report.manifests != null) {
        ...
        if (itemName == 'LOCATION' && getGPSCoordinates(activeManifest)) {
            return getGPSCoordinates(activeManifest);
        }
        if (itemName == 'WEBSITE') {
...
function getGPSCoordinates(manifest) {
  const exifAssertion = manifest?.assertions.get('stds.exif')?.[0]?.data;
  const longitude = exifAssertion?.['EXIF:GPSLongitude'];
  const latitude = exifAssertion?.['EXIF:GPSLatitude'];
